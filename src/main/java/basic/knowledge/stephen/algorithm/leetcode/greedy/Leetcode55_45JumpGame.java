...
//        System.out.println(flag);
    ...
    private boolean jumpB(int[] arr) {
        ...
        for (int i = 0; i < N; ++i) {
            //注意false的条件，就是maxreach停止了，而i仍然在增加，一直到超过maxreach也没有停止，
            // 对应题目中的反例很好理解, 这就是i i(不含)之前的所有位置起跳都到不了 i 的情况, 断层了 肯定是false
            if (i > maxreach) {
                return false;
            }
    ...
    private int jump(int[] arr) {
        ...
        int jump = 0;
        // curPos 注意是下标值，而不是元素值; 当前位置, 而非真实的跳跃下标 或 小于i 的 跳跃得最远位置
        // 是理论的跳jump次下的最远 最远  最远跳跃距离的下标
        //curPos i 遍历到i时 最远跳跃位置, nextPos 遍历i 之后的最远位置
        int curPos = 0;
